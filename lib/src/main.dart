...
class SleepStageChartPainter extends CustomPainter {
  ...
  void _drawHorizontalLines(Canvas canvas, Size size, Paint paint) {
    // 添加保护，防止因width和space总和 <= 0 导致无限循环
    ...
    if (allDayModel) {
      ...
      for (int i = 0; i <= verticalLineCount; i++) {
        ...
        while (startY < chartHeight) {
          // 使用 min 函数确保虚线终点不会超过图表高度
          final double endY =
              math.min(startY + verticalLineStyle.width, chartHeight);
          // 绘制垂直的线段 起点 (x, startY) 到 终点 (x, endY)
          canvas.drawLine(
            Offset(x, startY),
            Offset(x, endY),
            paint,
  ...
  void _drawTitle(Canvas canvas, Size size) {
    ...
    // 如果有信息 则直接用
    if (currentStage.info.isNotEmpty) {
      stageName = currentStage.info.join();
    }
    ...
    final durationSec =
        currentStage.endTime.difference(currentStage.startTime).inSeconds;
